...
printf 'https://cyber-atharv.com/crypha' > qr.txt               # the QR's visible content
```
...
Scan `stego-qr.png` with any phone and you get `https://cyber-atharv.com/crypha`. Point crypha at it and you get the secret. Auto-detect has to be careful here, because a QR stego is a PNG: it must not be mistaken for an ordinary image carrier. It is not.
